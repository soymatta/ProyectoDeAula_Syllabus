...
def create_evaluation():
    try:
        ...
        db.session.rollback()
        return (
            jsonify({"error": "Error al crear el evaluación", "details": str(e)}),
            400,
        )
...
def update_evaluation(id):
    try:
        ...
        if not evaluation:
            return jsonify({"error": "Evaluación no encontrado"}), 404
        ...
        return jsonify({"mensaje": "Evaluación actualizado correctamente"}), 200
    except Exception as e:
        db.session.rollback()
        return (
            jsonify({"error": "Error al actualizar el evaluación", "details": str(e)}),
            500,
        )
...
def delete_evaluation(id):
    try:
        ...
        return jsonify({"mensaje": "Evaluación eliminado correctamente"}), 200
    except Exception as e:
        db.session.rollback()
        return (
            jsonify({"error": "Error al eliminar el evaluación", "details": str(e)}),
            500,
        )
