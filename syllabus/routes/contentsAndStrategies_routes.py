...
from ..models.contentsAndStrategies_model import (
    ContentsAndStrategies,
    ContentsAndStrategiesSchema,
)
...
def create_contentAndStrategy():
    try:
        ...
        return (
            jsonify(
                {"error": "Error al crear el contenido y estrategia", "details": str(e)}
            ),
            400,
        )
...
def update_contentAndStrategy(id):
    try:
        ...
        if not contentAndStrategy:
            return jsonify({"error": "Contenido y estrategia no encontrado"}), 404
        ...
        return (
            jsonify({"mensaje": "Contenido y estrategia actualizado correctamente"}),
            200,
        )
    except Exception as e:
        db.session.rollback()
        return (
            jsonify(
                {
                    "error": "Error al actualizar el contenido y estrategia",
                    "details": str(e),
                }
            ),
            500,
...
def delete_contentAndStrategy(id):
    try:
        ...
        return (
            jsonify({"mensaje": "Contenido y estrategia eliminado correctamente"}),
            200,
        )
    except Exception as e:
        db.session.rollback()
        return (
            jsonify(
                {
                    "error": "Error al eliminar el contenido y estrategia",
                    "details": str(e),
                }
            ),
            500,
